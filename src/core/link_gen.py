...
def build_sub_ids(
    ...
    Args:
        ...
        timestamp: Timestamp YYYYMMDD_HHMM (gera automaticamente se None)
    ...
    sub_ids = ["tg", f"grupo{group_hash}", campaign_type, timestamp]

    if tag:
        # Trunca tag se necessário (max 255 chars)
        tag = tag[:255]
        sub_ids.append(tag)
